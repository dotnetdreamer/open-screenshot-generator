...
Lets the Open Screenshot Generator AI agent run on the Claude, ChatGPT, Gemini, Copilot, DeepSeek, Qwen or
...
Without the extension the agent still works. Open Screenshot Generator falls back to a manual relay: it copies
...
the reply, and hand the reply text back to Open Screenshot Generator.
...
Reload the Open Screenshot Generator tab. The agent's "Free, use my account" tab should now say
...
and `*.github.io`. If you host Open Screenshot Generator somewhere else, add that origin to
