...
- `gen-app-skeletons.js` — regenerates the neutral **skeleton-screen library** in `public/data/projects/app-screens/` used to fill empty phone mockups in the App Screenshots templates (the full-height counterpart to `fg-screens/`, in the inboxly-screen-inbox.png visual language: soft grey blocks, one indigo accent). Outputs 6 archetypes × {light,dark} — `app-<list|feed|grid|player|dashboard|chat>-<light|dark>.png` — at 846×1710 @ DPR2 (== the iphone-15 screen aspect, so `objectFit:cover` never crops). Edit the archetype builders / palettes in this file, then rerun from repo root: `node .claude/skills/app-screenshots/scripts/gen-app-skeletons.js`.
