...
/**
 * Portrait (1080x1920) layout of the MCP scene. Same beats as landscape,
 * but the rebuilt dialog is narrower and the connect command wraps onto
 * two lines so the zoom can frame it inside a phone canvas.
 */
...
const PILL = { x: 870, y: 1354, w: 146, h: 34 };
const CARD = { x: 220, y: 700, w: 640, h: 442 };
const CODE = { x: CARD.x + 30, y: CARD.y + 262, w: CARD.w - 60, h: 96 };
...
/** An empty artboard so the canvas reads as the editor before the dialog opens. */
...
/** The MCP connection dialog, narrowed for the portrait canvas. */
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "absolute", left: 30, bottom: 24, fontSize: 13.5, color: "#7A8483" }}>
        Design tools over local HTTP
      </div>
    </div>
  );
};

export const McpZoomSceneMobile: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
    ...
    <AbsoluteFill>
      <Camera
        keyframes={[
          ...
          { frame: 46, x: 540, y: 960, scale: 1 },
          { frame: 74, x: 540, y: 990, scale: 1.65 },
        ]}
      >
        <div
          style={{
        ...
        {frame >= 78 && (
          <div style={{ position: "absolute", left: CODE.x, top: CODE.y, width: CODE.w, height: CODE.h }}>
            <SelectionFrame appearAt={78} radius={8} handleSize={10} />
          </div>
        )}

        <Cursor
          path={[
            { frame: 10, x: 980, y: 1800 },
            { frame: 30, x: 943, y: 1371 },
            { frame: 52, x: 943, y: 1371 },
            { frame: 72, x: 808, y: 1010 },
          ]}
          clicks={[32, 75]}
          size={28}
        />
      ...
      >
        ...
        <Headline delay={3} size={60} align="center">
          {"Claude Code\ncan drive it"}
        </Headline>
      </div>

      <CaptionPill from={76} size={22} bottom={270}>
