...
4c. A colour **tint** over a picture is an SVG filter on the `<img>` (`imageTint` in [elementStyle.ts](../src/lib/elementStyle.ts), rendered by `ImageTintFilter`), never a coloured div over it. `feComposite operator="in"` against `SourceAlpha` tints only the pixels the browser painted, so every `objectFit` and every transparent PNG is handled without knowing the image's natural size. An overlay would paint the bars a `contain` leaves, turn a cut-out into a coloured rectangle, and change the silhouette the element's own drop-shadow is cast from. The filter def must live INSIDE the node being captured or it is missing from every export, and the id is derived from the tint's VALUES so it is identical in the clone. The video compositor has no filters, so it bakes the tint in once with `source-atop` before the frame loop.
