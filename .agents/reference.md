...
| "Use my API key" | `runApiMode` -> [generatePlan.ts](../src/lib/ai/generatePlan.ts), registry in [providers.ts](../src/lib/ai/providers.ts) | inline compact always, plus a real JSON Schema | `anthropic` needs `anthropic-dangerous-direct-browser-access`; `openrouter` and `compatible` must use `.chat()` |

**Any OpenAI-compatible endpoint (`compatible`).** The fifth provider is the escape hatch for a subscription we ship no entry for: the user names the base URL, the model id and (unless it is a local server) a key. `COMPATIBLE_PRESETS` in [providers.ts](../src/lib/ai/providers.ts) is a list of ready-made URLs, nothing more, and every field it fills stays editable. Three things make it work where a plain `createOpenAI` would not:

- **The endpoint is asked what it serves.** `listCompatibleModels()` reads `<base>/models`, because a model id typed from memory is the likeliest way to get a 404 that reads like an outage.
- **The request does not go through the browser.** Every call with a custom base URL uses `bridgeFetch` from [httpBridge.ts](../src/lib/ai/httpBridge.ts), which is `tauri-plugin-http` on desktop and so is not subject to CORS. Most third party hosts, and every localhost runtime, send no CORS headers for our origin. The scope in [capabilities/default.json](../src-tauri/capabilities/default.json) allows `https://*` for exactly this reason: the host is unknowable when that file is written. A scope refusal still falls back to the webview fetch, and note that tauri-plugin-http rejects with a plain **string**, not an Error, so any `instanceof Error` check on it silently misses (see `messageOf` in httpBridge.ts).
- **A schema is not assumed.** Plenty of compatible servers answer `response_format: json_schema` with a 400, or ignore it and reply in prose. `generatePlan` retries the run as a plain `generateText` turn carrying `buildJsonReplyInstruction()`, and mines the reply with `extractJsonCandidates`. The verdict is remembered in `localStorage` under `agent-endpoint-json:<baseUrl>|<model>` (per model, because a gateway serves one model that enforces schemas beside one that cannot), and the negative one is stamped and expires after 7 days, so a single 502 cannot pin an endpoint to the weaker path for good. It is only written when the failure was `bad-request`: a 5xx or a badly built plan says nothing about what the endpoint supports. The named providers never fall back at all, because a failure there is a real failure.
- **A key belongs to a host, not to the provider slot.** `keys.compatible` would be one slot for what is really any number of providers, so a key stored there would follow the user to the next endpoint they typed. Custom-endpoint keys live in `AiSettings.compatibleKeys`, keyed by host, and the panel swaps the field's contents whenever the endpoint changes (`adoptEndpoint`, on preset change and on blur of the URL field, never per keystroke).

A base URL is also settable for the four named providers (an "Advanced" field), for a proxy or a gateway. Setting one on `openai` switches it from the Responses API to `.chat()`, because a clone almost never implements the former.
...
- A custom endpoint's failure to honour the JSON Schema is retried as text, but only for a custom endpoint (`mayFallBack` is `endpoint.length > 0`). Giving the named providers the same retry would turn a real 400 into two requests and a vaguer error.
- `normalizeBaseUrl` trims a pasted `/chat/completions` tail and adds a missing scheme, and it runs on **every** read of the setting, not just on save: an endpoint stored by an older build is not normalized.
...
### The timeline bar (canvas playback + clip editing)

[playback.ts](../src/lib/video/playback.ts) is a module-level clock: one board plays at a time, one rAF loop, `playArtboard/togglePlayback/pausePlayback/seekPlayback/restartPlayback/stopPlayback` plus the `usePlaybackTime(artboardId, enabled)` / `usePlaybackRunning` / `usePlaybackActive` / `usePlaybackSnapshot` hooks. It evaluates the SAME `animationStateAt` / `gesturePhaseAt` the export does, so the player and the MP4 agree frame for frame.

- Entry points: the per-board toolbar Play button ([ArtboardToolbar.tsx](../src/components/open-screenshot-generator/ArtboardToolbar.tsx), shown when `artboardTimeline(board).hasMotion`) and [PreviewTimelineBar.tsx](../src/components/open-screenshot-generator/PreviewTimelineBar.tsx) — the full-width bar docked above the canvas tool pill, holding the transport, a second ruler, one clip per animated layer, and the playhead.
- **The bar belongs to the SELECTED board.** Selecting another board calls `stopPlayback()` (effect on `activeArtboardId`) and the bar disappears with it; Esc also exits. Both were bugs first: a paused board used to keep `pointer-events: none` with no visible way out, which read as "this artboard cannot be selected".
- **Every element gets a row**, not just animated ones. A layer with no animation renders as a dashed full-length ghost; dragging it (or its hover "+", or dropping the layer in from the Layers panel, which is `draggable` with `ELEMENT_DRAG_TYPE`) applies the default `fade` / 0.6s enter at that second and turns it into a real clip. A ghost's body drag deliberately behaves like a left-edge drag: it runs the whole board, so there is nothing to slide, and the clamp used to pin it in place.
- Clips are editable, video-editor style: drag sideways to retime, drag an edge to trim, drag up/down to restack (that one rewrites the elements array, i.e. z-order, via `onReorderElement`). `clipUpdates()` maps a dragged `[start, end]` back onto whatever the element actually stores — `enterDelay`/`exitStart` for animations (dragging the right edge in CREATES a fade exit), `triggerTime`/`gestureDuration` for gestures, `trimStart`/`trimEnd` for recordings. The in-flight drag lives in a REF, not state: reading state in the pointerup handler misses the last move and commits nothing.
- Loop length comes from [timeline.ts](../src/lib/video/timeline.ts) `artboardTimeline(ab)` — the synchronous twin of `analyzeArtboardForVideo` (it reads `el.durationSeconds` instead of awaiting the media table), same `Math.min(30, Math.max(1, ceil(end) || 15))` rule — unless the board carries `previewDurationSeconds`, the explicit length the bar's number field sets (1..`PREVIEW_DURATION_MAX`, "auto" clears it). The export dialog seeds its duration from the same field.
- **The artboard component must never subscribe to the clock.** It re-renders at 60fps if it does, which re-creates every element it hands down. Only leaves subscribe: `DraggableElement` (folds the state into its transform/opacity, and only when the element HAS an `animation`), `GestureElement` (swaps its CSS loop for a `<canvas>` drawn by `drawGesture`), the two video elements (via `useTimelineVideo`), and the transport bar. Elements arrive as a stable `children` prop, so React bails out on their subtrees.
- While a board is previewing it is a player, not an editor: the artboard div takes `pointer-events: none` and selection chrome is suppressed. Both export paths call `stopPlayback()` first, so a running timeline can never be rasterized mid-animation.
- `useTimelineVideo` hands recordings back and forth: idle they keep looping muted, previewing they are seeked to `trimStart + min(t, playable)` (the export's `sourceTimeAt`) and re-synced only past 0.25s of drift. A recording with `keepAudio` is unmuted only while its own board's transport runs and its trim has not run out. Never drop the `muted` JSX attribute: `StaticArtboard` renders the same components without a board id, so every preview dialog copy would play the sound at once.
- `video-device` also accepts `videoSrc` (asset path or `data:` URL) as well as an uploaded `mediaId`. mediaId wins. This is the only way footage travels inside a project file, since the Dexie media table does not: a `data:video/mp4;base64,...` of a few hundred KB is a perfectly good demo clip, and both the canvas player and the export compositor read it through the same `loadVideoSource`.
...
- **Audio is a mix, when the browser can encode it.** With WebCodecs `AudioEncoder` present, the MP4 carries a 48kHz stereo AAC track: sound layers (`audio` elements) plus the own sound of each recording with `keepAudio`, or silence. Without it (older WKWebView) `createAudioTrack` returns null and the file is video only, which App Store Connect rejects. WebKit's encoder hands back a whole ES_Descriptor as `decoderConfig.description`; `normalizeAacDescription` cuts it down to the AudioSpecificConfig, or AVFoundation cannot open the track.
- **Recording sound.** `keepAudio` unset is silent, so projects from before it stay silent. `uploadKeepsAudio` turns it on only for a layer that had no recording yet. A recording's sound runs from second 0 over its trim, like its picture, and stops where the picture holds its last frame. [mp4Audio.ts](../src/lib/video/mp4Audio.ts) pulls the AAC (or MP3) packets for the trimmed range out of an MP4/MOV with `Blob.slice` and decodes them as ADTS, because WebKit's `decodeAudioData` rejects QuickTime containers (every macOS screen recording) and a whole recording in an ArrayBuffer is issue #19 memory. WebM goes to `decodeAudioData` whole; an MP4/MOV whose sound cannot become ADTS (PCM, ALAC) only does under 64MB, and otherwise the export fails with a message pointing at the sound switch. Only the first edit of an edit list is honoured. Store modes keep the sound of the one recording they use.
- **Three store modes.** `rawRecordingOnly` renders the first recording full-bleed (guideline 2.3.4: "previews may only use video screen captures of the app itself"). With `keepOverlays` it also composites the artboard's TEXT and GESTURE layers, animations included, over that footage — the same guideline allows narration and textual overlays. Shapes, images, frames and the board background are excluded from both, and the filename suffix (`_AppPreview` / `_StoreReady_Text` / `_StoreReady`) keeps the three renders of one board apart.
...
`AccountSession` (`provider`, `account`, `accessToken`, optional `refreshToken` / `expiresAt`) is stored **unencrypted** in `localStorage` under `open-screenshot-generator.account` by [store.ts](../src/lib/account/store.ts). Reads go through `readWithLegacyFallback` and clears through `removeWithLegacy` ([legacyStorage.ts](../src/lib/legacyStorage.ts)) so the pre-rename `artboard-studio.account` key migrates on first read; a plain `removeItem` on the new key would let the legacy value come back. Only public client ids are compiled in, plus one deliberate exception: the desktop build carries the Google **Desktop-app** client secret, because Google's token endpoint rejects the installed-app exchange without it. That value is public by construction (it ships in every binary, as Google's own docs expect) and is not interchangeable with the Web client's secret, which must never be compiled in. `AccountAuthError` clears the stored session, `AccountCancelledError` is a user backing out and must not be toasted as a failure. **Except from the syncer**, which never clears a session: see below.

#### Keeping it up to date on its own

Off by default, switched on with `accountAutoSync` in [editorPreferences.ts](../src/lib/editorPreferences.ts). The state machine is [autoSync.ts](../src/lib/account/autoSync.ts) (`AccountAutoSyncer`), bound by [use-account-auto-sync.ts](../src/hooks/use-account-auto-sync.ts) and rendered by [AccountSyncChip.tsx](../src/components/open-screenshot-generator/account/AccountSyncChip.tsx). It is a deliberate near-copy of the cloud saver with five differences that are the whole point, and none of them may be "simplified" back:

1. **It only updates.** `syncProjectToAccount()` in `index.ts` is the only door, and it does nothing without a row in the Dexie `accountLinks` table ([links.ts](../src/lib/account/links.ts)). That row is minted by a manual save and by `loadProjectFromAccount`, never by the timer, so ticking the switch cannot create anything in somebody's storage. There is no `FIRST_PUSH_MS`.
2. **It never sweeps.** `AccountSaveOptions.sweepOrphans` is false for every unattended push. The sweep decides what to delete remotely from a local IndexedDB read, and an evicted browser makes that read a wrecking ball. `ProjectBundle.missingMedia` exists for the same reason and turns a manual save's sweep off too.
3. **It checks before it writes.** `CloudProvider.readRemoteStamp()` is one cheap request (Drive: `project.json`'s `version`; GitHub: the gist HEAD sha) and a mismatch throws `AccountConflictError` and stops. This can only *notice* a clobber: Drive v3 removed etags and accepts no `If-Match`, and a gist PATCH takes no precondition, so it is check-then-act by necessity. Watch the **document**, never the Drive folder, whose `modifiedTime` every rename moves.
4. **It never signs anybody out.** An `AccountAuthError` from a timer parks the chip at `needs-attention`; it does not call `setSession(null)` the way `handleAuthFailure` does.
5. **It is held during collab.** `collabActive` stops it entirely, because in a room the edit rate is set by however many people are typing and none of them owns the Drive quota.

Cadence lives in `CADENCE` in `autoSync.ts` and is per provider: Drive 8s quiet / 90s ceiling / 20s floor; GitHub 15s / 180s / 60s. Drive is only modestly slower than the cloud saver next door (5/30/8) and the margin is for its three requests and uncompressed body, not for whose disk it is: its quota is 325,000 units per minute per user and it prunes its own revisions, so there is no reason to be timid. GitHub's floor is the one with a real constraint behind it, and it is not the 5000/hour primary limit: a gist write is a **content-creating** request against a documented 500/hour secondary limit that is per USER and shared with their own github.com activity, so a tight floor could throttle their own pushes. 60s spends about 12% of it. Note `minGap` counts from the last push and a manual save counts as one, so the first automatic push after a hand save waits out the whole gap.
...
- GitHub **refuses** a bundle with `bundle.media.length` and points at Drive (`AccountBlockedError`, which the syncer treats as terminal rather than retrying). Do not "fix" that by dropping the blobs. This bites only projects with the user's own uploads: the shipped templates reference their art as plain `public/` paths, so a template-only project syncs to a gist fine.
- `bundle.missingMedia` lists ids the manifest references that IndexedDB no longer has. A manual save carries on with the orphan sweep disabled; an unattended one refuses outright, because an incomplete bundle plus a sweep deletes the last surviving copy.
...
- **The Next app router drops inline `<script>` from the root layout during static export.** [src/app/layout.tsx](../src/app/layout.tsx) carries none. When you need pre-hydration script (an anti-flash gate, say), render it from a client component next to the markup it guards, with `dangerouslySetInnerHTML`.
- **HTML5 drag and drop never fires for a finger**, and a coarse pointer sends no `mousedown` or `mouseover` either. The canvas is driven by pointer events throughout, palette tiles carry a second long-press drag path ([use-touch-drag.tsx](../src/hooks/use-touch-drag.tsx)), and hover-only affordances are revealed by `data-touch-reveal`. See rules 19 to 22 in [AGENTS.md](AGENTS.md).
