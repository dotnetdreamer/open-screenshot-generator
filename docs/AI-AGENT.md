...
```
https://editor.openscrgen.app/data/ai/catalog.txt
```

(That host is `PUBLIC_SITE_URL` in [hostedCatalog.ts](../src/lib/ai/hostedCatalog.ts); a fork
overrides it with `NEXT_PUBLIC_SITE_URL` at build time.)
